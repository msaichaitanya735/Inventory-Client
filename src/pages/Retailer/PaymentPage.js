    ...
    try {
      ...
      if (itemsForPayment && itemsForPayment.length > 0) {
        for (const item of itemsForPayment) {
          const response = await axios.post(`http://localhost:8080/order/orderproduct?retailerId=${localStorage.getItem('userId')}&productId=${item.productId}&quantity=${item.quantity}&method=${paymentType}&role=${localStorage.getItem('role')}&returnable=${item.returnable}`);

...
const OrderHistoryPage = () => {
  ...
  useEffect(() => {
    ...
    axios.get(`https://saichaitanyamuthyala.com/order/getorders?retailerId=${localStorage.getItem('userId')}`)
      .then(response => {
        ...
        const fetchProductDetails = async () => {
          const productResponses = await Promise.all(
            productIds.map(productId =>
              axios.get(`https://saichaitanyamuthyala.com/inventory/getproduct?productId=${productId}`)
            )
          );
  ...
  const handleInitiateReturn = async (orderId) => {
    try {
      const response = await axios.put(`https://saichaitanyamuthyala.com/order/initiateReturn`, null, {
        params: { orderId }
      });

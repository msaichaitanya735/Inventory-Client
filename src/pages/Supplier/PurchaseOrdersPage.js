...
const PurchaseOrdersPage = () => {
  ...
  useEffect(() => {
    const fetchPurchaseOrders = async () => {
      try {
        ...
        const response = await axios.get(`https://main.dwoh96qwfxa1j.amplifyapp.com/order/getpurchases?supplierId=${localStorage.getItem('userId')}`);
    ...
    const fetchAllProducts = async () => {
      try {
        ...
        // Replace with actual API endpoint
        const response = await axios.get('https://main.dwoh96qwfxa1j.amplifyapp.com/inventory/fetchallproducts');
  ...
  const handleAccept = async (purchaseID, quotationPrice) => {
    try {
      ...
      const api = `https://main.dwoh96qwfxa1j.amplifyapp.com/order/addquotationforpurchase?purchaseId=${purchaseID}&supplierId=${localStorage.getItem('userId')}&price=${quotationPrice}`;

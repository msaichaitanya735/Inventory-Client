...
const PurchaseOrdersPage = () => {
  ...
  useEffect(() => {
    const fetchPurchaseOrders = async () => {
      try {
        ...
        const response = await axios.get(`https://saichaitanyamuthyala.com/order/getpurchases?supplierId=${localStorage.getItem('userId')}`);
    ...
    const fetchAllProducts = async () => {
      try {
        ...
        // Replace with actual API endpoint
        const response = await axios.get('https://saichaitanyamuthyala.com/inventory/fetchallproducts');
  ...
  const handleAccept = async (purchaseID, quotationPrice) => {
    try {
      ...
      const api = `https://saichaitanyamuthyala.com/order/addquotationforpurchase?purchaseId=${purchaseID}&supplierId=${localStorage.getItem('userId')}&price=${quotationPrice}`;
